...
These encode the VAL-PYSDK-B01..B14 assertions for moss-langchain.
"""
import re
import sys
from pathlib import Path

from moss_langchain import uninstall
...
def _run(monkeypatch, args):
    monkeypatch.setattr(sys, "argv", ["moss_langchain.uninstall", *args])
...
def test_dry_run_exits_zero_and_no_changes(tmp_path, monkeypatch):
    """VAL-PYSDK-B01 / B05: --dry-run exits 0 and mutates nothing."""
...
def test_removes_only_moss_config_files(tmp_path, monkeypatch):
    """VAL-PYSDK-B02 / B09: real run removes MOSS config files, unrelated files byte-identical."""
    _populate_fixture(tmp_path)
    monkeypatch.chdir(tmp_path)
    before = _snapshot(tmp_path)
...
def test_env_prefix_key_matching(tmp_path, monkeypatch):
    """VAL-PYSDK-B03 / B10: only keys beginning with MOSS_ removed; others preserved."""
    _populate_fixture(tmp_path)
    monkeypatch.chdir(tmp_path)
...
def test_no_subdir_recursion(tmp_path, monkeypatch):
    """VAL-PYSDK-B11: .env files in subdirectories are not modified."""
    _populate_fixture(tmp_path)
    monkeypatch.chdir(tmp_path)
...
def test_manifest_stays_valid_only_moss_removed(tmp_path, monkeypatch):
    """VAL-PYSDK-B04 / B12: moss-sdk dep removed, manifest still valid, other deps intact."""
    _populate_fixture(tmp_path)
    monkeypatch.chdir(tmp_path)
    ...
    if tomllib is not None:
        ...
        assert any("langchain-core" in d for d in deps)
        assert any("httpx" in d for d in deps)


def test_requirements_txt_dependency_removed(tmp_path, monkeypatch):
    """VAL-PYSDK-B04: moss-sdk stripped from requirements.txt, other requirements kept."""
    ...
    assert "langchain-core>=0.1.0" in reqs
    assert "httpx>=0.27.0" in reqs


def test_idempotent_empty_dir(tmp_path, monkeypatch):
    """VAL-PYSDK-B08: nothing to clean -> no crash, exit 0, no files created/deleted."""
    monkeypatch.chdir(tmp_path)
...
def test_dry_run_real_parity(tmp_path, monkeypatch):
    """VAL-PYSDK-B13: the plan claimed by --dry-run equals what the real run acts on."""
    _populate_fixture(tmp_path)
    monkeypatch.chdir(tmp_path)
...
def test_prints_manual_checklist(tmp_path, monkeypatch, capsys):
    """VAL-PYSDK-B06 / B14: checklist enumerates the real un-automatable steps."""
    monkeypatch.chdir(tmp_path)
...
def test_no_network_imports():
    """VAL-PYSDK-B07: the helper performs no network/API calls (local-only)."""
